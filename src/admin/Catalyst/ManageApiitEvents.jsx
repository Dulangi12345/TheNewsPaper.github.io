...
const ManageApiitEvents = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    const docReference = doc(db, "Catalyst", "ApiitEvents");
      ...
      if (
        ...
        if (articleToEdit.articleImage instanceof File) {
          const storageRef = ref(
            storage,
            `CatalystArticleImages/ApiitEvents/${
              articleToEdit.articleTitle
            }-${Date.now()}`
          );
        ...
        if (
          articleToEdit.index !== null &&
          articleToEdit.index < ArticlesData.length
        ) {
          const updatedData = {
          ...
          console.error(
            "The specified index does not exist in the ArticlesData array."
          );
        }
...
export default ManageApiitEvents;
